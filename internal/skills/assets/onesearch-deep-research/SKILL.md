...
- Before executing returned `steps[].command`, run `onesearch status --format json` and use only available capabilities, provider filters, and provider-direct endpoints. If a planned command names an unavailable provider, replace it with `onesearch search`/workflow routing through an available provider or report the gap.
